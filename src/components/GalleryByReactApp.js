...
class ImgFigure extends Component {
	render() {
		...
		return(
			<figure data-index={this.props.index} className={imgFigureClassName} style={styleObj} onClick={this.props.handle().bind(this)}>
				<img src={this.props.data.imageURL}
						 alt={this.props.data.title}
				/>
				<figcaption>
					<h2 className="img-title">{this.props.data.title}</h2>
					<div className="img-back"  onClick={this.props.handle().bind(this)}>
...
class ControllerUnit extends React.Component {
	render() {
		...
		return (
			<span className={controllerUnitClassName} onClick={this.props.handle().bind(this)}></span>
		)
	}
}
...
class GalleryByReactApp extends React.Component {
		...
		for(var i = 0,j = imgsArrangeArr.length, k = j / 2;i < j; i++){
			...
			imgsArrangeArr[i] ={
				pos:{
					...
					left: this.getRangeRandom(hPosRangeLorR[0],hPosRangeLorR[1])
				},
				rotate:this.get30DegRandom(),
				isCenter: false
			}
	...
	render() {
		...
		})(require('../sources/imageDatas.json'));
